...
export default function Home() {
  ...
  const loadRun = (saved: SavedAnalysis) => { setWeatherOverride(null); setInputs(saved.result.inputs); setResult(saved.result); setSaveName(saved.name); toast.success(`Loaded ${saved.name}`); };
          ...
          <motion.aside initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .55, delay: .08 }} className="h-fit rounded-2xl border border-white/10 bg-[#0b1016]/90 p-5 shadow-[0_25px_70px_rgba(0,0,0,.25)] backdrop-blur-2xl xl:sticky xl:top-5" data-testid="simulation-controls-panel">
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-xl border border-sky-300/15 bg-sky-400/[.06] p-3" data-testid="weather-data-panel">
                ...
                <p className="mt-2 text-[10px] leading-5 text-slate-500" data-testid="weather-series-context">Day/night simulation inputs, separate from the OpenWeatherMap current conditions above.</p>
