...
import type { AnalysisRequest } from "@/lib/thermal-types";
import {
  Card,
  CardContent,
  CardDescription,
    ...
    <div className="min-h-screen overflow-hidden bg-[#05070a] text-slate-100">
      ...
      <main className="relative z-10 mx-auto min-h-[calc(100vh-140px)] max-w-[1600px] px-5 pb-12 pt-5 lg:px-8">
        <Tabs value={activeTab.id} onValueChange={value => navigateTab(String(value))} className="gap-0">
          ...
          {!validInputs && <p role="alert" className="mb-5 rounded-xl border border-rose-300/20 bg-rose-400/10 p-3 text-sm text-rose-200" data-testid="workspace-validation-error">Check your inputs: dimensions must be positive (length/width ≤30 m, height ≤15 m), daily solar irradiation &gt;0–12 kWh/m², and sunshine &gt;0–24 hours.</p>}
          ...
          <TabsContent value="analysis" className="workspace-enter mt-0 min-w-0" data-testid="results-workspace">
            ...
            {!result ? <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-5 flex min-h-[310px] flex-col items-center justify-center rounded-2xl border border-dashed border-white/15 bg-[#0b1016]/70 p-8 text-center backdrop-blur-xl" data-testid="empty-results-state"><div className="relative mb-6 flex h-16 w-16 items-center justify-center rounded-full border border-orange-300/20 bg-orange-400/10 text-orange-300"><Sun size={28} className="animate-[spin_12s_linear_infinite]" /></div><p className="font-heading text-2xl" data-testid="empty-results-title">The shelter is waiting for its sky.</p><p className="mt-2 max-w-lg text-sm leading-6 text-slate-400" data-testid="empty-results-description">Configure Geometry, Solar & climate, and Materials using the tabs, then run the model to reveal temperature retention and heat-loss pathways.</p><Button variant="outline" onClick={runAnalysis} disabled={!validInputs || analyzeMutation.isPending || aiMutation.isPending} className="mt-6 border-orange-300/25 bg-orange-400/10 text-orange-200 hover:bg-orange-400/20" data-testid="empty-run-analysis-button"><Play size={14} className="mr-2 fill-current" /> {analyzeMutation.isPending ? "Running model…" : `Run ${inputs.location} study`}</Button></motion.div> : <>
              ...
              <div className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(270px,.7fr)]"><Card className="overflow-hidden border-white/10 bg-[#0b1016]/85 shadow-[0_25px_80px_rgba(0,0,0,.22)] backdrop-blur-xl" data-testid="temperature-chart-card"><CardHeader className="flex-col items-start justify-between gap-3 border-b border-white/10 pb-4 sm:flex-row"><div><CardDescription className="font-mono text-[10px] uppercase tracking-[0.16em] text-slate-500" data-testid="temperature-chart-eyebrow">{result.inputs.duration_hours} hour thermal response / {result.inputs.location}</CardDescription><CardTitle className="mt-2 font-heading text-xl font-medium" data-testid="temperature-chart-title">Inside vs ambient temperature</CardTitle></div><div className="flex gap-3 pt-1 text-[10px] text-slate-400"><span className="flex items-center gap-1.5" data-testid="chart-legend-inside"><i className="h-1.5 w-5 rounded-full bg-orange-400" /> inside</span><span className="flex items-center gap-1.5" data-testid="chart-legend-ambient"><i className="h-1.5 w-5 rounded-full bg-sky-400" /> ambient</span></div></CardHeader><CardContent className="pt-5"><div className="h-[300px] w-full" data-testid="temperature-line-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 8, right: 10, left: -14, bottom: 0 }}><CartesianGrid stroke="rgba(148,163,184,.10)" vertical={false} /><XAxis dataKey="label" tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} tickLine={false} interval={Math.max(Math.floor(chartData.length / 6), 1)} /><YAxis tick={{ fill: "#64748b", fontSize: 10 }} axisLine={false} tickLine={false} unit="°" /><Tooltip contentStyle={{ background: "#101720", border: "1px solid rgba(255,255,255,.12)", borderRadius: 10, fontSize: 11 }} labelStyle={{ color: "#94a3b8" }} /><Line type="monotone" dataKey="inside_temp" name="Inside" stroke="#fb923c" strokeWidth={3} dot={false} activeDot={{ r: 5, fill: "#fb923c", stroke: "#0b1016", strokeWidth: 3 }} /><Line type="monotone" dataKey="ambient_temp" name="Ambient" stroke="#38bdf8" strokeWidth={2} strokeDasharray="5 5" dot={false} /></LineChart></ResponsiveContainer></div><div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-white/10 pt-4"><span className="text-xs text-slate-500" data-testid="temperature-chart-caption">Comfort target <span className="font-mono text-slate-300">{result.comfort_target}</span></span><span className="font-mono text-xs text-orange-300" data-testid="average-temperature">avg {result.average_inside_temp.toFixed(1)}°C inside</span></div></CardContent></Card>
